...
We've sliced the plan into bite-sized tasks, each with its own test. Now the agent actually starts writing — and this is where a lot of people make the same quiet mistake: they hit go, wander off, and come back to a finished diff. The diff is the *most* expensive place to discover the agent misread you. This chapter is about staying in the room while it works — steering with the concrete thing rather than a hand-wave, and accepting the slightly deflating fact that telling an agent a rule and getting the rule obeyed are two very different events.
...
> **Next up — [Verify — Proof, Not Vibes](05-verify-proof-not-vibes.md):** the code is written and the agent cheerfully reports "all tests passing." Lovely — but it wrote those tests from its own code, so they prove it does what it does, not what you asked for. Next we replace the warm feeling with actual evidence, graded by someone who isn't the author.
