...
The change is built and verified — proof attached, contract met. One step left before it's truly done, and it's the one fluency makes dangerous. Agent code reads *beautifully*: it compiles, it's tidy, it looks like a careful engineer wrote it. That polish is exactly the camouflage a bug hides behind. So this final lifecycle chapter does two things at once — read the diff like you mean it to catch the plausible-but-wrong, and pull shared shapes together before the third near-copy quietly drifts apart. Review keeps wrong code out; convergence keeps the right code from rotting.
...
> **Next up — [Guides — Feedforward](../20-harness/01-guides-feedforward.md):** that's one whole change shepherded from spec to merge. But you don't want to hand-hold every change forever — so the next section builds the *harness itself*, the reusable controls that do this steering for you. We start with guides: everything you put in front of the agent before it acts.
