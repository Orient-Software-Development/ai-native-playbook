...
This is page one, so there's nothing behind us yet — coat's still on, coffee's still hot. Here's the whole book in a breath: a coding agent is a blazingly fast pair of hands with no instinct for what you actually *meant*, and everything that follows is about building the system that keeps it pointed the right way. The very first thing to clear up is the phrase everyone assumes they already understand — "AI-native." It's not the autocomplete in your editor. Let's start there.
...
> **Next up — [Context Engineering](02-context-engineering.md):** the harness runs on one scarce resource, and the agent can only ever act on what it can *see* this exact instant. Time to meet the context window — a desk that's smaller than you'd like, and that you keep piling paper onto.
