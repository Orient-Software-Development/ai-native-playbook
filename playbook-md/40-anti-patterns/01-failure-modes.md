...
We've spent the whole book so far building things up — lifecycle, harness, delivery, the works. This is the chapter where we deliberately knock them over to watch how they shatter. Call it the rogues' gallery: every way AI-native development goes wrong, lined up and named. The twist is that it's secretly *one* villain in eight different costumes — the agent chasing "task-complete" instead of "task-correct," then writing prose smooth enough to hide the difference. Name the disguise and you can point straight at the practice that foils it.
...
> **Next up — [The Minimum Viable Harness](../50-adoption/01-minimum-viable-harness.md):** knowing every way it breaks is bracing, but it's not much help on a Monday morning. So the final section turns practical: how to actually stand all this up from wherever you happen to be today. We start with the smallest harness that still closes the loop — the one you could have running by Friday.
