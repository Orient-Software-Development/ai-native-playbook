...
Three pages of guides behind us — feedforward, everything that shapes the agent before it acts. Guides nudge the odds; they never actually *enforce* anything. So now we build the half that does: sensors, the checks that grade the work after the fact. But there's a catch that quietly hollows out most test suites you'll ever meet, and it's worth saying before anything else — a check the agent can pass *without doing the work* is not a sensor. It's decoration that everybody trusts.
...
> **Next up — [Keep Quality Left](04-keep-quality-left.md):** you've got a layered gate of honest sensors. The obvious next question — and the one teams reliably fumble — is *where each check runs*. Try to run everything as early as possible and you'll build a pre-commit hook so slow the whole team quietly learns to skip it.
