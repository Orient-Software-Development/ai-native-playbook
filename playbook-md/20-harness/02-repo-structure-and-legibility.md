...
Last page covered the guides you sit down and author — instruction files, blueprints, constraints. This one is sneakier, because the most powerful guide in your repo is one you never actually write. It's the *layout*. Before the agent reads a single word you wrote, it reads the folder tree, the file names, where everything sits relative to everything else. A tidy, conventional structure is feedforward the repo broadcasts for free, on every task, forever. A bespoke mess is a maze it has to grope through one file at a time.
...
> **Next up — [Sensors — Feedback](03-sensors-feedback.md):** that's the whole feedforward half of the harness — everything that shapes the agent *before* it acts. Time to flip the coin to the other half: sensors, the checks that grade the work *after* — and the one rule underneath them that decides whether any of them are real.
