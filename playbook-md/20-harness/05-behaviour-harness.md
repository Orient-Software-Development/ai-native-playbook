...
Foundations named three things a harness regulates, hardest last: is the code clean, is it the right shape, and — the genuinely brutal one — does it do what the business actually *wanted*? The first two you can check against the code itself. The third you can't, because the code *is* the agent's answer to that question, and asking the agent to test its own answer is exactly how you end up with a suite that's a mirror, not a check. This page is the way out, and it hinges on one word: bind the test to the **invariant**, not the implementation.
...
> **Next up — [Trunk-Based Development](../30-delivery/01-trunk-based-development.md):** that completes the harness — guides before, sensors after, behaviour pinned to intent. You can now turn out correct change after correct change. The next section is about *shipping* them at the speed the agent unlocks, without the integration pile-up that speed usually drags in. First stop: one shared line of code, kept relentlessly green.
