...
> **Start here instead:** this raw prompt is the manual fallback. The maintained, skill-driven version is the
> [Harness Adoption Starter](../../harness-adoption/README.md) — its [`assess` skill](../../harness-adoption/skills/assess.md)
> wraps this same discipline (discover → measure → score) with stack detection and per-tool entry points
> (Claude Code, Cursor/Copilot/Codex, or a [paste prompt](../../harness-adoption/adapters/paste-prompt.md) for
> chat agents with no repo access). Use the block below only when you want the bare prompt with no tooling.
