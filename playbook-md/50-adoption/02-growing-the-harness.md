...
Last page got you a small-but-whole harness standing by Friday. This page is every week after. The temptation now is to sit down and architect the Complete Harness in one grand session — resist it, because you genuinely cannot predict how your agent will go wrong, so any blueprint is a guess. Instead you run a loop: it fails once, you ask what control was missing, you encode the fix so that exact failure can never happen again. A mature harness *looks* designed; it was actually grown, one failure at a time. And — the motion teams always forget — occasionally pruned.
...
> **Next up — [The Responsible Team and AI Debt](03-responsible-team-and-ai-debt.md):** growing the harness makes the agent more *capable*. It does nothing to make it *responsible* — it has no career on the line and no recoil at a 300-line function. Next we cover the two jobs only the team can do: deciding what stays human, and treating the debt the agent generates like the high-interest loan it is.
